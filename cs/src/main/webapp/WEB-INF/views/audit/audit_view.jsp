	...
	<div style="border: #95B8E7 1px solid; width: 900px;">
		<table class="company-examined" border="0" cellspacing="0" cellpadding="0">
			<tr>
				...
				<td width="300" colspan="2"><input name="companyCode" class="bj_belu2" type="text" value="${entity.company.companyCode}" /> <input type="hidden" id="companyId" name="company.id"
					value="${entity.company.id}" /> <input type="hidden" id="year" value="${entity.year}" /> <input type="hidden" name="company.version" value="${entity.company.version}" /></td>
				<td>年审年度:</td>
				<td><input name="year" type="text" value="${entity.year}" /></td>
				<td>未审年度:</td>
				<td><input type="text" <c:if test="${unAudityearNum==0}">value="无" </c:if> <c:if test="${unAudityearNum>0}">value="${unAudityear}" class="error"</c:if> />
				</td>
			</tr>
			<tr>
				<td>企业名称:</td>
				...
				<td><input type="text" value="${ageEx }" <c:if test="${ageEx>0}">class="error"</c:if> <c:if test="${ageEx==0}"></c:if> /></td>
			</tr>
		...
		<table class="company-examined" border="0" cellspacing="0" cellpadding="0">
			...
			<tr>
				...
				<td style="vertical-align: bottom" rowspan="2"><input name="unauditYears" id="weiShenNianShu" title="未审年数" style="border-top: #95B8E7 2px solid;" value="${unAudityearNum}" />
				</td>
				<td style="vertical-align: bottom" rowspan="2"><input name="supplementYear" title="补审年度" style="border-top: #95B8E7 2px solid;" value="${supplementYear}" /></td>

			</tr>
			<tr>

				<td><input type="text" class="text_short" value="${entity.companyShouldTotal}" name="companyShouldTotal" id="yingAnPaiCanJiRen" title="应按排数" /></td>
				<td><input type="text" class="text_short" value="${entity.companyAlreadyTotal}" name="companyAlreadyTotal" id="yiAnPaiCanJiRen" title="已按排数" /></td>
				<td><input id="yiLuRuCanJiRen" name="companyHandicapTotal" title="已录入残疾职工人数" value="${entity.companyHandicapTotal }" /></td>
				<td><input id="yuDingCanJiRen" name="companyPredictTotal" value="${entity.companyPredictTotal }" title="预定残疾职工人数" /></td>
				<td><input name="remainAmount" id="shangNianDuWeiJiaoBaoZhangJin" title="上年度未缴保障金额度" value="${entity.remainAmount}" /></td>
			</tr>
		...
		<table class="company-examined" border="0" cellspacing="0" cellpadding="0">
			<tr>
				...
				<td><input id="yingJiaoJinE" type="text" name="amountPayable" value="${entity.amountPayable}" /></td>
				<td width="100">减缴金额:</td>
				<td width="100"><input id="jianJiaoJinE" type="text" name="reductionAmount" value="${entity.reductionAmount}" /></td>
				<td width="99">免滞纳金:</td>
			...
			<tr>
				<td>实缴金额:</td>
				<td><input id="shiJiaoJinE" type="text" name="actualAmount" value="${entity.actualAmount}" />
				</td>
				<td>滞纳金:</td>
				<td><input id="zhiNaJin" type="text" name="delayPayAmount" value="${entity.delayPayAmount}" />
				</td>
				<td>实缴总金额:</td>
				<td colspan="1"><input id="shiJiaoZongJinE" type="text" name="payAmount" value="${entity.payAmount}" />
				</td>
				<td width="91">滞纳金天数:</td>
				<td><input name="delayDays" id="zhiNaJinTianShu" value="${entity.delayDays}" /></td>
			</tr>
			<tr>
				<td width="100">备注:</td>
				<td colspan="8"><textarea name="remark" rows="2" style="height: 60px;">${entity.remark }</textarea></td>
			</tr>
		</table>
		<!-- 年审企业表格  第六部分(意见栏) -->
		<table class="company-examined" border="0" cellspacing="0" cellpadding="0">
			<tr>
				...
				<td colspan="3"><textarea name="initAuditComment" rows="3" cols="45" style="height: 60px;">${entity.initAuditComment}</textarea></td>
				<td width="100">复审意见:</td>
				<td colspan="3"><textarea rows="3" cols="45" style="height: 60px;">${entity.verifyAuditComment}</textarea></td>
			</tr>
			<tr>
				<td class="">初审日期:</td>
				<td><c:if test="${entity.initAuditDate==null}">
					...
					</c:if> <input value="${date}" /></td>
				<td width="100">初审人:</td>
				<td><input type="text" value="${entity.initAuditUser.userRealName}" /></td>
				<td class="">复核日期:</td>
				<td><fmt:formatDate value="${entity.verifyAuditDate}" type="both" dateStyle="long" pattern="yyyy-MM-dd" var="verifyAuditDate" /> <input type="text" class="bj_belu4"
					value="${verifyAuditDate}" /></td>
				<td width="100">复审人:</td>
				<td><input type="text" value="${entity.verifyAuditUser.userRealName}" /></td>
			</tr>
		</table>
	</div>
