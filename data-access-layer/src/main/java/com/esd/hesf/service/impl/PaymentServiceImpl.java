...
public class PaymentServiceImpl implements PaymentService {
	...
	@Override
	public BigDecimal getAlreadyPayByAccounts(Integer accountsId) {
		if (accountsId == null || accountsId <= 0) {
			new HesfException("accountsId", HesfException.type_number_negative)
					.printStackTrace();
			return null;
		}
		Payment payment = new Payment();
		payment.setAccounts(new Accounts(accountsId));
	...
	@Override
	public PaginationRecordsAndNumber<Payment, Number> getPaymentRecordByAccounts(
			Integer accountsId, Integer page, Integer pageSize) {
		if (accountsId == null || accountsId <= 0) {
			new HesfException("accountsId", HesfException.type_number_negative)
					.printStackTrace();
			return null;
		...
		t.setAccounts(new Accounts(accountsId));
		map.put("payment", t);
		// 起始索引值
		map.put("start", page <= 1 ? Constants.START : (page - 1) * pageSize);
		// 返回量
