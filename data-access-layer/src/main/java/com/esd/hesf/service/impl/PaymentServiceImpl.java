...
public class PaymentServiceImpl implements PaymentService {
	...
	@Override
	public PaginationRecordsAndNumber<Payment, Number> getPaymentRecords(
	...
	@Override
	public PaginationRecordsAndNumber<Payment, Number> getPaymentRecords(
			String year, String companyCode, Integer page, Integer pageSize) {
		// 将参数放入到map中
		Map<String, Object> map = new HashMap<String, Object>();
		Payment t = new Payment();
		t.setYear(year);
		t.setPaymentCompany(new Company(companyCode));
		map.put("payment", t);
		// 起始索引值
		map.put("start", page <= 1 ? Constants.START : (page - 1) * pageSize);
		// 返回量
	...
	public Boolean createPaymentExcel(String FilePath,
		...
		for (int i = 1 + startRow; i <= (page - 1) * pageSize
			...
			row.createCell(35).setCellValue(
					KitService.getStringDate(payment.getPaymentDate()));
			// 缴款金额
			...
			if (payment.getBillPrintDate() != null) {
				row.createCell(40).setCellValue(
						KitService.getStringDate(payment.getBillPrintDate()));
			}
			// 换票日期
			if (payment.getBillExchangeDate() != null) {
				row.createCell(41)
						.setCellValue(
								KitService.getStringDate(payment
										.getBillExchangeDate()));
			}
			// 是否返票
